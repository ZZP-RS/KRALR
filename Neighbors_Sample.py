...
file = open(save_path + '/trainCKG.txt','a')
...
for key in path_dict.keys():
    ...
    item_num_list = [] # 存储每一条路径中的物品数量
    mean_popularity_list = [] # 存储每一条路径的平均物品流行度
    lower_popularity_list = []  # 平均物品流行度最低的路径list
    neighbors = []
    _str = ""

    for p in path_list:#p为一条路径
        popularity = 0
        n = 0
        for entity_id in p:
            if entity_id < n_items and entity_id in train_item_dict.keys():
                n += 1
